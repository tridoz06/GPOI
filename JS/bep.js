...
function calculateBEP(fixedCost, variableCost, salePrice) {
  return fixedCost / (salePrice - variableCost);
}
...
function drawGraph() {
  ...
  if (salePrice <= variableCost) {
    alert('Il prezzo di vendita deve essere maggiore dei costi variabili per calcolare il BEP.');
    return;
  }
  ...
  ctx.stroke();
  ctx.font = '14px Arial';
  ctx.fillStyle = '#fff'; // Impostato il colore del testo su bianco
  ctx.fillText('Unità', 750, 370);
  ctx.fillText('Costi / Ricavi', 10, 50);

  const xStep = 700 / maxUnits;
  const yStep = 300 / maxCost;
  ctx.strokeStyle = '#fff'; // Impostato il colore del testo su bianco
  ctx.font = '12px Arial';
